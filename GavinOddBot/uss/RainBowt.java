...
public class RainBowt extends AdvancedRobot
{
    ...
    public class Tank implements RobotPart
    {
        ...
        public void move()
        {
            //double wallGap = 100;
            /*
             * if ((getBattleFieldWidth() - getX() < wallGap ||
             * getBattleFieldHeight() - getY() < wallGap || getX() < wallGap ||
             * getY() < wallGap )&& pass == 0) { if (pass == 0) { pass = 100; }
             * moveDirection *= -1; setTurnRight(10 * moveDirection);
             * out.println("stuck?"); }
             */

//            if ( getBattleFieldWidth() - getX() < 50 )
//            {
//                setTurnLeft( normalizeBearing( getHeading() + 30 ) );
//                setAhead( 50 * moveDirection );
//                out.println( "too close to right" );
//            }
//            if ( getX() < 50 )
//            {
//                setTurnLeft( normalizeBearing( getHeading() - 30 ) );
//                setAhead( 50 * moveDirection );
//                out.println( "too close to left" );
//            }
//            if ( getBattleFieldHeight() - getY() < 50 )
//            {
//                setTurnLeft( normalizeBearing( getHeading() + 120 ) );
//                setAhead( 50 * moveDirection );
//                out.println( "too close to top" );
//            }
//            if ( getY() < 50 )
//            {
//                setTurnLeft( normalizeBearing( getHeading() - 120 ) );
//                setAhead( 50 * moveDirection );
//                out.println( "too close to bottom" );
//            }
            
             //When the tank gets too close to the right side.
             if (getBattleFieldWidth() - getX() < 50) {
                 if (enemy.getY() > getY()) {
                     setTurnRight(normalizeBearing(enemy.getBearing() - 90 + 10));
                 } else if (enemy.getY() < getY()) {
                     setTurnRight(normalizeBearing(enemy.getBearing() + 90 - 10));
                 }
                 out.println("Too close to the right wall");
                 setAhead(75);
                 execute();
             } 
             
             // When the tank gets too close to the left side
             else if (getX() < 50) {
                 if (enemy.getY() > getY()) {
                     setTurnRight(normalizeBearing(enemy.getBearing() + 90 - 10));
                 } else if (enemy.getY() < getY()) {
                 ...
                 out.println("Too close to the left wall");
                 setAhead(75);
                 execute();
             }
            
             //When the tank gets too close to the top side
             else if (getBattleFieldHeight() - getY() < 50) {
                 if (enemy.getX() > getX()) {
                     setTurnRight(normalizeBearing(enemy.getBearing() + 90 - 10));
                 } else if (enemy.getX() < getX()) {
                     setTurnRight(normalizeBearing(enemy.getBearing() - 90 + 10));
                 }
                 out.println("Too close the upper wall");
                 setAhead(75);
                 execute();
             } 
            
             //When the tank gets too close to the bottom side
             else if (getY() < 50) {
                 if (enemy.getX() > getX()) {
                     setTurnRight(normalizeBearing(enemy.getBearing() - 90 + 10));
                 } else if (enemy.getX() < getX()) {
                 ...
                 out.println("To close to the bottom wall");
                 setAhead(75);
                 execute();
             }
             else /*if ( enemy.getDistance() < 100 || ram )
                 {
                 ram = true;
                 moveRam();
             }*/
